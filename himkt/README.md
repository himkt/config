...
The policy contains integer `version: 1` and `allow`/`block` arrays. Each rule is an ordered argument array. `**` matches zero or more whole arguments; `*` inside another element matches characters within one argument. Every other character is literal. Matching is case-sensitive and covers the whole argv. Block rules win, and unmatched commands are blocked. Maintain Bash permissions here, including the CAFleet shell-prompt exceptions. Claude settings retain permissions for other tools. Codex rules separately control sandbox escalation; they cannot authorize commands rejected by this policy.
